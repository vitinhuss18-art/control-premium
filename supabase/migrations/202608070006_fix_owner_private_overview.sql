...
-- Qualifica todas as colunas dos agregados para que os nomes dos parametros
-- de saida da funcao PL/pgSQL nao sejam interpretados como colunas locais.
...
begin
  ...
  left join (
    select
      ...
      sum(loan.principal_cents) filter (where loan.status <> 'cancelled') as total_principal_lent_cents
  ...
  left join (
    select
      ...
      sum(installment.total_cents - installment.paid_cents) as overdue_amount_cents
