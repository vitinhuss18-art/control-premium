...
-- Bug real: decide_client_proposal() cria/atualiza o cliente mas nunca devolvia
-- loan_amount_cents. O front-end (index.html, decidirPropostaExterna) dependia
-- de um cache local em memoria do navegador (propostasExternasCache) pra saber
-- quanto emprestar na chamada seguinte (POST /api/admin/loans). Se esse cache
-- estivesse vazio ou desatualizado no momento da aprovacao -- pagina recarregada,
-- aba reaberta, sessao restaurada, corrida entre dois carregamentos da lista --
-- o cliente era criado normalmente mas o emprestimo nao, com falha silenciosa
-- (so um aviso na tela, sem retry). Foi o que aconteceu no caso real da cliente
-- Mayara Christina Ramos: proposta aprovada, cliente criado, emprestimo nao.
--
-- Correcao: a RPC passa a devolver loan_amount_cents (que ja esta em
-- client_proposals, nunca muda apos o envio da proposta) junto com os demais
-- dados do cliente, na mesma resposta atomica. O front-end deixa de depender
-- do cache local para esse valor.
drop function if exists public.decide_client_proposal(uuid, text, text, integer, integer, text);

create function public.decide_client_proposal(
  p_proposal_id uuid,
  p_decision text,
